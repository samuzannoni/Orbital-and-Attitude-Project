...
h_N_target = [0; 0; I(3,3)*n]; % Target angular momentum in body frame
...
% Control torque
Kp = 5*1e-3*ones(length(w_0),1);
Kd = 5*1e-4*ones(length(w_0),1);
w_target = [0; 0; n];
...
% 
% % Actuator torque
% actuators_torque = simulation.actuators;