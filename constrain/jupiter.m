...
n1 =(t_dep_fin-t_dep_in+1)*2-1; % step time span
...
toc
% se runno per giorni ci mette 18 secondi, se runno per mezza giornata 110
% secondi, se runno ogni 6 ore per 590sec cioe 9 minuti

%[DV_tot2,min_DV_tot2,date12,date22] = Deltavtot(3,5,t_span_dep,t_span_arr,mu_S,n1,n2);
% dato che per giove il vincolo è verificato, controllo il risultato con la funzione Deltavtot normsle

%% PORKCHOP PLOT

figure
[X1, X2] = meshgrid(t_span_dep, t_span_arr);
contour(X1,X2,DV_tot',[14.6 15],'LineWiàdth', 1) % 'ShowText', 'on');
hold on;
